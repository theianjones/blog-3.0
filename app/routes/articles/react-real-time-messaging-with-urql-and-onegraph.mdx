...
>
  <h3 style={{marginBottom: 0, marginTop: 10, fontSize: 24}}>{issue.title}</h3>
  <p
    style={{
      marginTop: 5,
      marginBottom: 10,
...
return (
  ...
  >
    <IssueList
      onLoaded={handleIssueListLoaded}
      onItemClick={(i) => console.log('click') || setCurrentIssue(i)}
    />
    {currentIssue && (
      <div style={{marginLeft: 20, maxWidth: 600, minWidth: 400}}>
        +{' '}
